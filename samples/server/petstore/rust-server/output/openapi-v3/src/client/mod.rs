    ...
    Conn: hyper::client::connect::Connect + Clone + Send + Sync + 'static,
{
    ...
    async fn xml_other_put(&mut self, param_string: Option<crate::models::AnotherXmlArray>, context: &Ctx) -> Result<XmlOtherPutResponse, ApiError> {
        let mut uri = format!(
            "{}/xml_other",
            self.base_path
        );
    ...
    async fn xml_post(&mut self, param_string: Option<crate::models::XmlArray>, context: &Ctx) -> Result<XmlPostResponse, ApiError> {
        let mut uri = format!(
            "{}/xml",
            self.base_path
        );
